/*
 * This file is the file https://download.affectiva.com/js/3.2/affdex.js
 * modified by Andrea Esposito <a.esposito39@studenti.uniba.it>. It has been
 * modified because the original file wasn't working on a self-hosted website
 * (localhost) in modern browsers (tested with Google Chrome v. 80.0.3987.87 [64
 * bit] and Mozilla Firefox v. 72.0.2 [64 bit]).
 *
 * Every edit is highlighted with a comment with three slashes and a tilde 
 * ('/// ~')
 */
...
//Global namespace
var affdex = affdex || {};
window.affdex = affdex; /// ~ Added global reference
affdex.version = "3.2.1.583-b86b1d2"
affdex.getAffdexDotJsLocation = function () {
    var scripts = document.getElementsByTagName('script');
    var affdexJS = null;
    /// ~ The source must be hardcoded because the file is now self-hosted.
    let src = "https://download.affectiva.com/js/3.2/";
    ...
    affdexJS = src.replace(/\/affdex\.js$/, '/');
    return affdexJS;
};
...
function XHRWorker(url, ready, scope) {
    ...
    */
    ...
    oReq.addEventListener('load', function () {
        /// ~ Modified from window.URL to a ternary operator.
        var worker = new Worker((window.URL? window.URL: window.webkitURL).createObjectURL(new Blob([this.responseText])));
        if (ready) {
